...
uint8_t pidcomputing(volatile struct pidparam *pid, int16_t speed, int16_t cur_speed){
	...
	int16_t Up = pid->error * pid->Kp;

	//Calculating integral gain
	int16_t Ui = pid->istate * (pid->Kp / (pid->Ti * pid->Ts));
	...
	int16_t Ud = (pid->Kp * pid->Td * pid->Ts) * (pid->error - pid->prev_error);

	//Error in previous sample time
	pid->prev_error = pid->error;

	int8_t control_signal = (Up + Ui + Ud) / 100 * 1.344;
